...
class ApplicationController < ActionController::Base
  ...
  # In the case that it can't find a user with a certain ID (database reset),
  ...
  private
    ...
    def require_medical_history
      if  current_user.medical_history.nil? &&
          ...
          current_user.subscription.product == Subscription::PREMIUM_SUBSCRIPTION
        redirect_to new_account_medical_history_path
      end
    end

    def require_paid_access
      if current_user.subscription.nil? || current_user.subscription.product == Subscription::FREE_SUBSCRIPTION
        redirect_to food_log_index_path
      end

      if current_user.subscription && Subscription.paid_subscription(current_user.subscription.product) && current_user.subscription.state.nil?
