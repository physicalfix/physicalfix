module AccountHelper
  ...
  def subscription_free_and_freeby?
    Subscription::VALID_FREE_PLANS.include?(session[:plan]) && Subscription::VALID_FREEBY_PLANS.include?(session[:plan]) && session[:key]
  end
  
  def subscription_freeby?
    Subscription::VALID_FREE_PLANS.include?(session[:plan]) && Subscription::VALID_FREEBY_PLANS.include?(session[:plan]) && session[:key]
  end
